...
const loginValidationSchema = yup.object().shape({
  ...
  email: yup
    ...
    .test(
      ...
      function () {
        // Use this.parent to access sibling fields in the same object
...
const Login = () => {
  ...
  // initialize with the first country as default
  ...
  // removed stray debug expressions that caused linter errors
  ...
  const onLoginSubmit = async () => {
    try {
      ...
      console.log("onLoginSubmit inputs", {
        phoneNumber,
        email,
        selectedCountry,
      });
      if (email) {
        ...
        // eslint-disable-next-line no-console
        console.log("sendOtp response (email):", response);
        ...
        // eslint-disable-next-line no-console
        console.log("sendOtp response (phone):", response);
        if (response.status === "success") {
  ...
  const onOtpSubmit = async () => {
    try {
      setLoading(true);
      if (!userPhoneData) {
        setError("Phone or Email data is missing");
        setStep(1);
        setLoading(false);
        return;
      }
      ...
      console.log("verifyOtp payload:", {
        phoneNumber: userPhoneData?.phoneNumber ?? null,
        phoneSuffix: userPhoneData?.phoneSuffix ?? null,
        email: userPhoneData?.email ?? null,
        otp: otpString,
      });

      let response;
      if (userPhoneData?.email) {
        // email flow: (phoneNumber, phoneSuffix, email, otp)
        response = await verifyOtp(null, null, userPhoneData.email, otpString);
      } else {
        // phone flow: (phoneNumber, phoneSuffix, email, otp)
        ...
        {step === 2 && (
          <form onSubmit={handleOtpSubmit(onOtpSubmit)} className="space-y-4">
            ...
            >
              ...
              {userPhoneData?.phoneSuffix ?? "Email"}{" "}
          ...
          >
            <div className="flex flex-col items-center mb-4">
              <div className="relative w-24 h-24 mb-2">
                <img
                  src={profilePicture || selectedAvatar || avatars[0]}
                  alt="profile"
